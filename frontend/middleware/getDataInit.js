  ...
  if (!hasInitdData) {
    ...
    for (const dataRef of dataToStoreAtInitList) {
      ...
      if (dataRef.from === 'rawObject') {
        // log && console.log('-MW- getDataInit / rawObject / dataRef.from :', dataRef.from)
        ...
        const initDataFromObjectPromise = new Promise((resolve) => {
          ...
          // log &&
          //   console.log(
          //     '-MW- getDataInit / rawObject / dataset.id',
          //     dataset.id,
          //     ' / res :',
          //     resp
          //   )
      ...
      if (callableFrom.includes(dataRef.from)) {
        // log && console.log('-MW- getDataInit / from URL / dataRef.from :', dataRef.from)
        ...
        const initDataFromUrlPromise = axios
          ...
          .catch((err) => {
            console.log(
              '-MW- getDataInit / from URL / error while loading from dataRef.url :',
              err
            )
            console.log('-MW- trying to load fro backupUrl now...')
            const backupPromises = []
            if (dataRef.backupUrl) {
