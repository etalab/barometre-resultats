      ...
      if (getDistant.includes(configsFrom)) {
        ...
        const initConfigFromURL = axios
          ...
          .catch((err) => {
            log &&
              console.log(
                '-MW- getConfigInit / getDistant / error - configRef.url : ',
                configRef.url
              )
            console.log(
              '-MW- getConfigInit / getDistant / error while loading from configURL.url :',
              err
            )
          })
        promisesArray.push(initConfigFromURL)
      } else if (configsFrom === 'local_js_files') {
        // log && console.log('-MW- getConfigInit / local_js_files / configRef.data.help : ', configRef.data.help)
        const initConfigFromJSfile = new Promise((resolve) => {
          const resp = configRef.data
          resolve(resp)
        })
          .then((resp) => {
            // log && console.log("-MW- getConfigInit / local_js_files / resp : ", resp)
            const configRefData = {
              field: configRef.field,
              data: resp
            }
          ...
          .catch((err) => {
            console.log(
              '-MW- getConfigInit / local_js_files / error while loading from configRef :',
              err
            )
          })
