...
const RUN_MODE = process.env.NUXT_ENV_RUN_MODE_OVERIDE || process.env.NUXT_ENV_RUN_MODE
console.log('>>> nuxt.config.js (start) / process.env.NUXT_ENV_RUN_MODE_OVERIDE : ', process.env.NUXT_ENV_RUN_MODE_OVERIDE)

console.log('>>> nuxt.config.js (start) / RUN_MODE : ', RUN_MODE)

const APP_VERSION = 'v.2.3.0 - map blocks'

// - - - - - - - - - - - - - - - - - - - - - - - -
// STRUCTURE FROM...
var { dataStructure } = require('./nuxt_loadStructure_fromJSON.js')
// console.log( '>>> nuxt.config.js (start) / dataStructure : ', dataStructure )
const routesToGenerate = []
for (const kpiFamily of dataStructure) {
  const kpiFamilyUrlAliases = kpiFamily.kpiFamilyUrlAliases
  routesToGenerate.push(...kpiFamilyUrlAliases)
}
// console.log( '>>> nuxt.config.js (start) / routesToGenerate : ', routesToGenerate )

// - - - - - - - - - - - - - - - - - - - - - - - -
// CONFIGS FROM...
var { configsReferences } = require('./nuxt_loadConfigs_fromJS.js')
...
if (RUN_MODE === 'dev') {
  ...
  // console.log('>>> nuxt.config.js / copyConfigsToJSON / staticJSONpath : ', staticJSONpath)

  for (const conf of configsReferences) {
  ...
  const jsonStructureFilename = 'dataStructure.json'
  const jsonStructureStr = JSON.stringify(dataStructure, null, 2)
  fs.writeFileSync(`${staticJSONpath}/${jsonStructureFilename}`, jsonStructureStr, 'utf8')
  console.log(
    '>>> nuxt.config.js / copyConfigsToJSON / end ... jsonStructureFilename : ',
    jsonStructureFilename
  )
}

// - - - - - - - - - - - - - - - - - - - - - - - -
// CONFIGS FROM...
const staticBaseForConfigJSON = `/configs/${staticConfigFolder}/json/`
console.log('>>> nuxt.config.js / process.env.NUXT_ENV_CONFIG_FROM : ', process.env.NUXT_ENV_CONFIG_FROM)
if (process.env.NUXT_ENV_CONFIG_FROM === 'local_json_files') {
  configsReferences = [
    ...
    {
      field: 'configAppKpiTables',
      url: staticBaseForConfigJSON + 'configAppKpiTables.json'
    },
    {
  ...
  configsReferences = [
    ...
    {
      field: 'configAppKpiTables',
      url: urlBase + process.env.NUXT_ENV_configAppKpiTables
    },
    {
      field: 'configAppNavbarFooters',
...
const configApp = {
  ...
  // ROUTES
  generateRoutes: routesToGenerate,
...
export default {
  ...
  head: {
    ...
    link: [
      ...
      { rel: 'icon', type: 'image/x-icon', href: '/favicon-fr_gouv/favicon.ico' }
  ...
  css: [
    ...
    // '~/assets/css/vue-simple-markdown.min.css'
  ],
   ...
   */
  plugins: [
    '~/plugins/odmComponents',
    { src: '~/plugins/mapbox', mode: 'client' },
    // { src: '~/plugins/simpleMarkdown', mode: 'client' },
