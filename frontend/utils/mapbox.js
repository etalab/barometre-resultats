...
export function transformDataset (source, dataset, geoCanvas, debug = false) {
  // debug && console.log("\n",  "+ + +".repeat(10))
  // debug && console.log("+ + + transformDataset ... source : ", source)
  // debug && console.log("+ + + transformDataset ... dataset.id : ", dataset.id)
  // debug && console.log("+ + + transformDataset ... dataset : ", dataset)
  // debug && console.log("+ + + transformDataset ... geoCanvas : ", geoCanvas)
  const transformedData = {
    type: 'FeatureCollection',
    features: []
  }
  const transformTo = source.transformTo
  // debug && console.log("+ + + transformDataset ... transformTo : ", transformTo)

  const canvasKey = transformTo.canvasKey
  // debug && console.log("+ + + transformDataset ... canvasKey : ", canvasKey)

  const srcKey = transformTo.srcKey
  // debug && console.log("+ + + transformDataset ... srcKey : ", srcKey)

  const features = []

  dataset.data.forEach((item) => {
    // debug && console.log("\n+ + + transformDataset ... item : ", item)
    const key = `${canvasKey.canvasKeyPrefix}${canvasKey.ignoreItem ? '' : item[srcKey]}${canvasKey.canvasKeySuffix}`
    // debug && console.log("+ + + transformDataset ... key : ", key)
    const itemPoint = canvasKey.keyIsFieldName
      ? geoCanvas[key]
      : geoCanvas.find((g) => g[canvasKey.field] === key)
    if (itemPoint) {
      // debug && console.log("+ + + transformDataset ... itemPoint : ", itemPoint)
      const properties = buildProperties(transformTo.properties, item, debug)
      // debug && console.log("+ + + transformDataset ... properties : ", properties)
      const geoObject = {
        type: 'Feature',
      ...
      // debug && console.log("+ + + transformDataset ... geoObject : ", geoObject)
      features.push(geoObject)
    }
  })
  transformedData.features = features
  // debug && console.log("\n+ + + transformDataset ... transformedData : ", transformedData)
  return transformedData
}

// GEOJSON PROPERTIES
export function buildProperties (propertiesArray, item, debug = false) {
  const properties = {}
  propertiesArray.forEach((prop) => {
    // debug && console.log("+ + + buildProperties ... prop : ", prop, '... '.repeat(5))
  ...
  // debug && console.log("+ + + buildProperties ... properties : ", properties)
  return properties
}
